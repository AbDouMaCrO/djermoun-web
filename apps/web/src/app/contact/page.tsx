...
export default function ContactPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-24">
      <h1 className="text-center text-3xl font-bold text-slate-900 sm:text-4xl">{c.heading}</h1>
      ...
      <div className="mt-16 grid gap-12 lg:grid-cols-2">
        <div className="space-y-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-slate-900">{c.officeValue}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-slate-900">{c.portValue}</p>
            </div>
          </div>
        ...
        <form className="space-y-4 rounded-xl border border-slate-200 bg-white p-6">
          <div>
            <label className="text-xs font-medium text-slate-600">{c.form.name}</label>
            <input
              type="text"
              className="mt-1.5 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-amber-500"
            />
          </div>

          <div>
            <label className="text-xs font-medium text-slate-600">{c.form.email}</label>
            <input
              type="email"
              className="mt-1.5 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-amber-500"
            />
          ...
          <div>
            <label className="text-xs font-medium text-slate-600">{c.form.phone}</label>
            <input
              type="tel"
              className="mt-1.5 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-amber-500"
            />
          ...
          <div>
            <label className="text-xs font-medium text-slate-600">{c.form.message}</label>
            <textarea
              rows={4}
              className="mt-1.5 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-amber-500"
            />
