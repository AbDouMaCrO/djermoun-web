...
export default function CarCard({ car }: { car: CarCardData }) {
  ...
  return (
    <div className="group overflow-hidden rounded-2xl border border-slate-200 bg-white transition-all duration-300 hover:border-amber-400/60 hover:shadow-lg dark:border-white/5 dark:bg-[#111827] dark:hover:border-amber-500/30 dark:hover:shadow-xl dark:hover:shadow-amber-500/5 hover:-translate-y-0.5">
      {/* Clickable image + info */}
      <Link href={detailHref} className="block">
        <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-[#0A0F1E]">
          ...
          <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-white to-transparent dark:from-[#111827]" />
          {/* Top badges */}
          <div className="absolute left-3 top-3 flex gap-2">
            ...
            >
              {car.condition === "new" ? dict.carCard.new : dict.carCard.used}
            </span>
            {isNew && (
              <span className="rounded-full bg-amber-100 text-amber-700 dark:bg-white/10 dark:text-amber-300 backdrop-blur-sm px-2.5 py-1 text-xs font-semibold">
                {dict.carCard.recent}
              </span>
            )}
          </div>
        ...
        <div className="px-5 pt-4 pb-2">
          <h3 className="text-slate-900 font-bold text-base text-center dark:text-white">
            {car.year} {car.make} {car.model}{car.engine ? ` — ${car.engine}` : ""}
          </h3>

          <div className="mt-3 flex flex-wrap justify-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="flex items-center gap-1">
              <Fuel size={13} className="text-slate-400 dark:text-slate-500" />
              {car.fuel ?? "—"}
            </span>
            {car.transmission && (
              <span className="flex items-center gap-1">
                <Cog size={13} className="text-slate-400 dark:text-slate-500" />
                {car.transmission}
              </span>
            )}
            {car.paint_condition && (
          ...
          <div className="mt-4 text-center">
            <p className="text-2xl font-black text-amber-600 dark:text-amber-400">{formatPrice(totalUSD)}</p>
      ...
      <div className="flex gap-2 px-5 pb-5 pt-2">
        ...
        {WA_NUMBER && (
          <a
            ...
            className="flex items-center gap-1.5 rounded-xl border border-green-200 bg-green-50 text-green-700 hover:bg-green-100 dark:border-white/10 dark:bg-white/5 dark:text-green-400 dark:hover:bg-green-500/10 dark:hover:border-green-500/30 px-3 py-2.5 text-sm font-semibold transition-colors"
          >
