...
export default function CountrySelector({ className }: { className?: string }) {
  ...
  return (
    <select
      ...
      className={`rounded-md border border-slate-300 bg-white text-slate-900 cursor-pointer dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:[&>option]:bg-[#0A0F1E] px-2 py-1 text-sm font-medium ${className ?? ""}`}
    >
